...
// Lazy initialization function for RAG service
async function initializeRAGIfNeeded() {
  if (DISABLE_RAG) {
    return false;
  }
  
  if (!ragService) {
    console.log('🚀 Lazy initializing RAG service...');
  ...
  if (!ragInitialized) {
    try {
      ...
      console.log('🛑 Falling back to simple responses');
      return false;
    }
  }
...
router.post('/message', authenticate, async (req, res) => {
  try {
    ...
    if (DISABLE_RAG) {
      ...
      const ragReady = await initializeRAGIfNeeded();
      
      if (!ragReady) {
        // Fallback to simple responses if RAG failed to initialize
        const answer = 'I apologize, but the intelligent response system is currently unavailable. Please contact the college administration for assistance.';
...
router.get('/stats', authenticate, async (req, res) => {
  try {
    ...
    // Get RAG service stats only if initialized
    let ragServiceStats = { notInitialized: true };
    if (ragService && ragInitialized) {
      ragServiceStats = ragService.getStats();
    }
    
    res.json({ overall: stats[0] || {}, byPlatform: platformStats, byLanguage: languageStats, ragServiceStats });
