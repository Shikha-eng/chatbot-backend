...
class LanguageService {
  ...
  async initialize() {
    try {
      ...
      // Test translation API availability (with timeout)
      const testPromise = this.testTranslationAPI();
      ...
      try {
        ...
        console.warn('⚠️ Translation API not available:', error.message);
        console.log('💡 Language service will use rule-based detection only');
      }
      
      console.log('✅ Language Service initialized');
    } catch (error) {
      console.error('❌ Language service initialization failed:', error);
      console.log('💡 Language service will work with basic functionality');
    }
  }

  /**
   * Test translation API availability
   */
  async testTranslationAPI() {
    try {
      await translate('hello', { to: 'hi' });
      return true;
    } catch (error) {
